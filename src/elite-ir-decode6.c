...
int main( int argc, char *argv[] )
{
    ...
    float energy, n_imp;
    float pulsecount;
    float seconds, millisecs;
//declare number of pulses for a KW

    if ( argc < 2 || strlen ( argv[1] ) == 0 )
    {
        fprintf ( stderr, "Missing input, use argument of '-' for stdin or filename to read\nUse 2nd argument to define pulse rate\n" );
	 fprintf ( stderr, "\nExample: elite-ir-decode - 1000\n");
        return -1;
    }
		    ...
		    if ( d.byte == 11 )
		       ...
		       time (&currtime);
	               ...
	               strftime( tmpbuf, MAX_BUF_SIZE,"%d/%m/%y %R,%s", ptoday);
                       pulsecount = ( d.buf[d.byte-3]);
		       seconds = (d.buf[d.byte-2]);
                       millisecs = (d.buf[d.byte-1]);
                       }
        ...
        else if ( s < 0 )
        {
		  ...
		  if ( pulsecount < 3 ) 
			...
			energy = ((pulsecount/n_imp) * (3600/30));
                       ...
                       pFile = fopen ("energylog.txt", "w");
                       fprintf(pFile, "efergy %s",tmpbuf);
                       fprintf(pFile, ",%2.3f\n", energy);       
                       fclose(pFile);  
		       printf ("\n efergy %s",tmpbuf);	
